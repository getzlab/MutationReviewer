...
class IGV_remote:
    ...
    def _load_w_index(self, bam_urls, bai_urls):
        if len(bam_urls) < 1:
            raise ValueError("Please provide at least one URL to load")
        if len(bam_urls) != len(bai_urls):
            raise ValueError("bam_urls and bai_urls are not the same length.")
            
        for i in range(len(bam_urls)):
            cmd = f"load {bam_urls[i]} index={bai_urls[i]}"
            self._send(cmd)
            self._adjust_viewopts()
