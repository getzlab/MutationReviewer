...
class GeneralMutationData(Data):
    def __init__(
        ...
        end_pos_col = None,
        more_mutation_groupby_cols: list = None,
        ...
        end_pos_col = end_pos_col if end_pos_col is not None else start_pos_col
        bam_cols = [bam_cols] if not isinstance(bam_cols, list) else bam_cols
        
        bai_cols = [] if bai_cols is None else bai_cols
        bai_cols = [bai_cols] if not isinstance(bai_cols, list) else bai_cols
        
        required_mutations_df_cols = [mutations_df_sample_col, chrom_col, start_pos_col, end_pos_col]
        if not set(
        ...
        required_bam_df_cols = [bam_df_sample_col] + bam_cols
        if not set(
        ...
        self.more_mutation_groupby_cols = more_mutation_groupby_cols
        self.group_muts_cols = [chrom_col, start_pos_col] + more_mutation_groupby_cols
